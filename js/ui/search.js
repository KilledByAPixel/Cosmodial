...
// Flat search index. Entries: { label, type: 'star'|'constellation'|'body'|'dso'|'comet'|'planet-moon'|'iss',
...
export function buildSearchIndex(stars, figures, bodyLabels, dsos = [], comets = [], moons = [], satellites = []) {
  ...
  // Always indexed, even before (or without) live TLE data — selecting it opens the card, which
  // explains itself when there's no position to fly to.
  for (const s of satellites) index.push({ label: s.label, type: 'iss', ref: s.label, aliases: s.aliases || [] });
  return index;
}
...
const TYPE_HINT = { constellation: 'constellation', body: '', star: 'star', dso: 'deep sky', comet: 'comet', iss: 'satellite' };
