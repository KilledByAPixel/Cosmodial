...
import { makeObserver, altAzOfStar, altAzOfBody, precessToDate } from '../js/core/astro.js';
...
test('Polaris altitude ~= observer latitude (canonical correctness check)', () => {
  ...
  const t = Astronomy.MakeTime(new Date('2025-06-06T06:00:00Z'));
...
test('Sun is below horizon at local midnight and above at local noon', () => {
  ...
  const midnight = altAzOfBody(Astronomy.Body.Sun, obs, Astronomy.MakeTime(new Date('2025-03-20T00:00:00Z')));
  const noon = altAzOfBody(Astronomy.Body.Sun, obs, Astronomy.MakeTime(new Date('2025-03-20T12:00:00Z')));
...
test('precession is identity at J2000 and ~1.4 deg per century', () => {
  const atEpoch = precessToDate(0, 0, Astronomy.MakeTime(new Date('2000-01-01T12:00:00Z')));
  assert.ok(sepDeg(0, 0, atEpoch.ra, atEpoch.dec) < 0.02, 'should be ~identity at J2000');

  const after100y = precessToDate(0, 0, Astronomy.MakeTime(new Date('2100-01-01T12:00:00Z')));
